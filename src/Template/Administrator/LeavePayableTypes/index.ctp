      ...
      <div class="col-xs-12">
        ...
        <div class="box no-border">
          <div class="box-header">
            <h3 class="box-title">
              <?= $this->Html->link(__('<i class="fa fa-plus-circle"></i> Add New Leave Payable Types'), [
                    'action' => 'add'
                  ],[
        ...
        <div class="box-body table-responsive no-padding">
          <table class="table table-hover table-bordered">
            ...
            <?php foreach ($leavePayableTypes as $leavePayableType): ?>
              <tr>
                <td><?= $this->Number->format($leavePayableType->id) ?></td>
                <td><?= h($leavePayableType->title) ?></td>
                <td class="text-center">
                  <?php if($leavePayableType->status == 'active'){ ?>
                ...
                <?= $this->Html->link(__('<i class="fa  fa-eye"></i>'), ['action' => 'view', $leavePayableType->id],['class'=>'label label-success','escape'=>false]) ?>
                <?= $this->Html->link(__('<i class="fa  fa-edit"></i>'), ['action' => 'edit', $leavePayableType->id],['class'=>'label label-info','escape'=>false]) ?>
                <?= $this->Form->postLink(__('<i class="fa  fa-close"></i>'), ['action' => 'delete', $leavePayableType->id], ['class'=>'label bg-red','confirm' => __('Are you sure you want to delete # {0}?', $leavePayableType->id),'escape'=>false]) ?>
                </td>
              </tr>
            <?php endforeach; ?>
            </table>
