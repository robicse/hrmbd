      ...
      <div class="col-xs-12">
        ...
        <div class="box no-border">
          <div class="box-header">
            <h3 class="box-title">
              <?= $this->Html->link(__('<i class="fa fa-plus-circle"></i> Add New Leave Packages'), [
                    'action' => 'add'
                  ],[
        ...
        <div class="box-body table-responsive no-padding">
          <table class="table table-hover table-bordered">
            <tr class="custom-table-header">
              ...
              <th class="text-center"><?= $this->Paginator->sort('casual') ?></th>
              <th class="text-center"><?= $this->Paginator->sort('sick') ?></th>
              <th class="text-center"><?= $this->Paginator->sort('earned') ?></th>
            ...
            <?php foreach ($leavePackages as $leavePackage): ?>
              <tr>
                <td><?= $this->Number->format($leavePackage->id) ?></td>
                ...
                <td class="text-center">
                  <?php if($leavePackage->status == 'active'){?>
                ...
                <?= $this->Html->link(__('<i class="fa  fa-eye"></i>'), ['action' => 'view', $leavePackage->id],['class'=>'label label-success','escape'=>false]) ?>
                <?= $this->Html->link(__('<i class="fa  fa-edit"></i>'), ['action' => 'edit', $leavePackage->id],['class'=>'label label-info','escape'=>false]) ?>
                <?= $this->Form->postLink(__('<i class="fa  fa-close"></i>'), ['action' => 'delete', $leavePackage->id], ['class'=>'label bg-red','confirm' => __('Are you sure you want to delete # {0}?', $leavePackage->id),'escape'=>false]) ?>
                </td>
              </tr>
            <?php endforeach; ?>
            </table>
