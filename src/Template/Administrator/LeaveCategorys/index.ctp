      ...
      <div class="col-xs-12">
        ...
        <div class="box no-border">
          <div class="box-header">
            <h3 class="box-title">
              <?= $this->Html->link(__('<i class="fa fa-plus-circle"></i> Add New Leave Categorys'), [
                    'action' => 'add'
                  ],[
        ...
        <div class="box-body table-responsive no-padding">
          <table class="table table-hover table-bordered">
            ...
            <?php foreach ($leaveCategorys as $leaveCategory): ?>
              <tr>
                <td><?= $this->Number->format($leaveCategory->id) ?></td>
                <td><?= h($leaveCategory->title) ?></td>
                <td class="text-center">
                  <?php if($leaveCategory->status == 'active'){ ?>
                    <i class="fa fa-check-square" style="color:green;"></i>
                  <?php }else{ ?>
                    <i class="fa fa-times-circle" style="color:red;"></i>
                  <?php } ?>
                </td>
                <td><?= h($leaveCategory->created) ?></td>
                <td><?= h($leaveCategory->modified) ?></td>
                <td class="actions text-right">
                <?= $this->Html->link(__('<i class="fa  fa-eye"></i>'), ['action' => 'view', $leaveCategory->id],['class'=>'label label-success','escape'=>false]) ?>
                <?= $this->Html->link(__('<i class="fa  fa-edit"></i>'), ['action' => 'edit', $leaveCategory->id],['class'=>'label label-info','escape'=>false]) ?>
                <?= $this->Form->postLink(__('<i class="fa  fa-close"></i>'), ['action' => 'delete', $leaveCategory->id], ['class'=>'label bg-red','confirm' => __('Are you sure you want to delete # {0}?', $leaveCategory->id),'escape'=>false]) ?>
                </td>
              </tr>
            <?php endforeach; ?>
            </table>
