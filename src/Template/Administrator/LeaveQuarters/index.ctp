      ...
      <div class="col-xs-12">
        ...
        <div class="box no-border">
          <div class="box-header">
            <h3 class="box-title">
              <?= $this->Html->link(__('<i class="fa fa-plus-circle"></i> Add New Leave Quarters'), [
                    'action' => 'add'
                  ],[
        ...
        <div class="box-body table-responsive no-padding">
          <table class="table table-hover table-bordered">
            <tr class="custom-table-header">
              ...
              <th class="text-center"><?= $this->Paginator->sort('start_date') ?></th>
              <th class="text-center"><?= $this->Paginator->sort('end_date') ?></th>
              <th class="text-center"><?= $this->Paginator->sort('casual_duration') ?></th>
              <th class="text-center"><?= $this->Paginator->sort('sick_duration') ?></th>
              <th class="text-center"><?= $this->Paginator->sort('earned_duration') ?></th>
              <th class="text-center"><?= $this->Paginator->sort('total_duration') ?></th>
            ...
            <?php foreach ($leaveQuarters as $leaveQuarter): ?>
              <tr>
                <td><?= $this->Number->format($leaveQuarter->id) ?></td>
                ...
                <?= $this->Html->link(__('<i class="fa  fa-eye"></i>'), ['action' => 'view', $leaveQuarter->id],['class'=>'label label-success','escape'=>false]) ?>
                <?= $this->Html->link(__('<i class="fa  fa-edit"></i>'), ['action' => 'edit', $leaveQuarter->id],['class'=>'label label-info','escape'=>false]) ?>
                <?= $this->Form->postLink(__('<i class="fa  fa-close"></i>'), ['action' => 'delete', $leaveQuarter->id], ['class'=>'label bg-red','confirm' => __('Are you sure you want to delete # {0}?', $leaveQuarter->id),'escape'=>false]) ?>
                </td>
              </tr>
            <?php endforeach; ?>
            </table>
